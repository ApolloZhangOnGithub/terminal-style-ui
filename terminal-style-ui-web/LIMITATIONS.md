...
渲染核心来自 pi（MIT）的 pi-tui / pi-coding-agent 0.80.7 并做了定制，源码在 `upstream/`（只收打包用到的文件），第三方依赖锁定版本由 npm 安装。`npm run vendor` 由源码生成 `vendor/`，CI 每次验证与提交的一致。本库通过包装 `renderToken`（代码块行号栏、块缓存、源码映射）扩展渲染，升级上游时要跑一遍测试确认这些挂钩仍然成立。
