...
/**
 * fork: 渲染异常隔离 —— 组件 render() 抛错时记日志并跳过，绝不让一个坏组件
 * uncaughtException 拖垮整个 TUI。（0.80.7 时代旧 override 的 D0100 保护；0.84.1
 * 官方版没有此保护，rebase 丢失后 markdown 代码块 null 返回值直接杀掉了整个进程。）
 */
const RENDER_ERROR_LOG_LIMIT = new Map();
export function renderErrorLog(code, message) {
    const key = `${code}:${String(message).slice(0, 120)}`;
    const count = (RENDER_ERROR_LOG_LIMIT.get(key) ?? 0) + 1;
    RENDER_ERROR_LOG_LIMIT.set(key, count);
    if (count > 20)
        return; // 同类错误最多记 20 次，避免每帧重绘刷爆日志
    try {
        const dir = path.join(os.homedir(), ".terminal-style-ui", "agent");
        fs.mkdirSync(dir, { recursive: true });
        fs.appendFileSync(path.join(dir, "genshin-debug.log"), `[${new Date().toISOString()}] ${code} ${message}\n`);
    }
    catch {
        // 日志写失败不影响主流程
    }
...
export class Container {
    ...
    _renderChildren(width) {
        const lines = [];
        for (const child of this.children) {
            // fork: 单组件渲染失败 → 记日志（D0100）并跳过该组件，保 TUI 存活
            try {
                // fork 2026-08-20: child 可能非组件（字符串/原始值）——防御性渲染，字符串作单行
                const childLines = typeof child.render === "function" ? child.render(width) : (typeof child === "string" ? [child] : []);
                for (const line of childLines) {
                    lines.push(line);
                }
            }
            catch (e) {
                renderErrorLog("D0100", e?.stack || String(e));
            }
        }
        return lines;
    }
...
export class TuiBase extends Container {
    ...
    constructor(terminal, showHardwareCursor, logDirectory) {
        ...
        this.logDirectory = logDirectory ?? process.env.GENSHIN_CODING_AGENT_DIR ?? process.env.PAIMON_CODING_AGENT_DIR ?? path.join(os.homedir(), ".terminal-style-ui", "agent");
    ...
    /**
     * fork 2026-09-13：探测终端给全角序号 ①②③（EAW=ambiguous）分配几格。
    ...
    requestRender(force = false) {
        if (process.env.PAIMON_HEADLESS === "1") return; // fork: headless 调试开关（0.80.7 时代定制）
        if (force) {
            this.resetRenderState();
    ...
    scheduleRender() {
        if (process.env.PAIMON_HEADLESS === "1") return; // fork: headless 调试开关
    ...
    handleTerminalInput(data) {
        if (process.env.INPUTPS05 === "0") return; // fork: 输入协议调试开关
        ...
        // fork 2026-09-13：全角序号格数探测的 CPR 回包（只在探测窗口内截获，不影响正常按键）
        ...
        if (this.focusedComponent?.handleInput) {
            ...
            if (process.env.INPUTPS06 !== "0") this.focusedComponent.handleInput(data); // fork: 输入协议调试开关
            ...
            if (process.env.INPUTPS07 !== "0") this.requestImmediateRender(); // fork: 输入协议调试开关
        }
    }
