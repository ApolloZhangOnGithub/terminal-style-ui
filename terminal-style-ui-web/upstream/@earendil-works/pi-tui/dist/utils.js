...
// fork 2026-08-14（ISSUE 092）：Intl.Segmenter.segment() 每次调用创建/销毁 ICU BreakIterator → 长会话 GC 风暴。
// 缓存包装：segment() 返回切分结果数组（for...of 兼容，调用方零改动）；LRU 缓存短文本（≤50 字符），长文本不缓存防内存膨胀；行为完全一致。
const _rawGraphemeSegmenter = new Intl.Segmenter(undefined, { granularity: "grapheme" });
const _rawWordSegmenter = new Intl.Segmenter(undefined, { granularity: "word" });
const _segCacheMax = 256;
const _segCacheMaxLen = 50;
const _graphemeSegCache = new Map();
const _wordSegCache = new Map();
function _segmentsWithCache(raw, cache, text) {
  if (text.length <= _segCacheMaxLen) {
    const hit = cache.get(text);
    if (hit) return hit;
    const arr = [...raw.segment(text)];
    if (cache.size >= _segCacheMax) {
  ...
  return [...raw.segment(text)];
}
const graphemeSegmenter = { segment: (text) => _segmentsWithCache(_rawGraphemeSegmenter, _graphemeSegCache, text) };
const wordSegmenter = { segment: (text) => _segmentsWithCache(_rawWordSegmenter, _wordSegCache, text) };
/**
...
const WIDTH_CACHE_SIZE = 8192; // fork: 0.80.7 时代定制（原 512），长会话减少宽度缓存抖动
...
/**
 * fork 2026-09-13：全角序号的"两格契约"输出侧（见 graphemeWidth 里的注释）。
 ...
 */
// fork 2026-08-15（ISSUE 092 续）：wrapTextWithAnsi 行级 memo——"窗口没变化就不从头渲染"
// 的落点。折行结果由 (text, width) 唯一决定，此前每帧对 transcript 全部行重跑
// Intl.Segmenter 分词（sample 实测主线程 74% 在 flatMap→Segmenter 链），元凶就是
// 没变化的行每帧都从零重折。LRU 缓存；命中返回浅拷贝（调用方会就地改数组元素，
// 如 hangWrapText 的 "/"-断点复原）；超长文本不缓存防内存膨胀。
const _wrapCache = new Map();
const _wrapCacheMax = 4096;
const _wrapCacheMaxLen = 1024;
export function wrapTextWithAnsi(text, width) {
    const key = width + "\u0000" + text;
    const hit = _wrapCache.get(key);
    if (hit !== undefined) {
        // 刷新 LRU 位置；浅拷贝返回，保护缓存不被调用方就地修改
        _wrapCache.delete(key);
        _wrapCache.set(key, hit);
        return [...hit];
    }
    const result = _wrapTextWithAnsiUncached(text, width);
    if (text && text.length <= _wrapCacheMaxLen && result.length > 0) {
        if (_wrapCache.size >= _wrapCacheMax) {
            const oldest = _wrapCache.keys().next().value;
            if (oldest !== undefined) _wrapCache.delete(oldest);
        }
        _wrapCache.set(key, result);
    }
    return result;
}
function _wrapTextWithAnsiUncached(text, width) {
    if (!text) {
        return [""];
    }
 ...
 */
// fork: 提取行尾的 ANSI SGR reset + OSC 8 close 序列（不可见序列链）
// —— 空格补在它们之前，避免尾部空格落在 OSC 8 close 之后导致部分终端超链接下划线异常
...
export function applyBackgroundToLine(line, width, bgFn) {
    ...
    // Apply background to content + padding（fork: 尾部不可见序列留在空格之后）
