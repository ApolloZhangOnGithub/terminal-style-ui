...
export class ScrollView extends Container {
    ...
    scrollbarHideTimer;
    // fork: 滚动跟随状态变化回调（FollowHint 提示条用）
    // 任何路径（键盘 scrollBy/scrollTo、滚轮、新内容自动跟随、布局变化）离开/回到底部都会触发。
    onFollowChange;
    onScrollActivity;
    _lastFollowingEnd;
    constructor(component, options = {}) {
        ...
        this.onScrollActivity = options.onScrollActivity;
        this._lastFollowingEnd = this.followingEnd;
    }
    get scrollTop() {
    ...
    _notifyFollowChange() {
        if (this.followingEnd !== this._lastFollowingEnd) {
            this._lastFollowingEnd = this.followingEnd;
            this.onFollowChange?.(this.followingEnd);
        }
    }
