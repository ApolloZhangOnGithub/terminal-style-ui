import { execSync, spawn } from "node:child_process"; // ISSUE 132 (2026-09-06 fork): 选中复制系统剪贴板工具 fallback
...
export class TuiAltScreen extends TuiBase {
    ...
    afterTerminalStop(options) {
        ...
        if (options.preserveScreen) {
            ...
            // fork ISSUE 091 终版（2026-08-15，用户终端为 kitty，用户明确要求）：
            // 退出后用户要看到的是"最后一帧原样"——live 渲染的 previousScreen
            // （完整 transcript + footer），而不是 stop 时刻重新 render 的残缺帧，
            // 更不能清掉上文只留重渲染的垃圾。
            // kitty：退出 alt screen 时原生把 alt 内容（连同 scrollback）保存进主
            // scrollback——不重渲染、不打印、不清屏，只退出，天然只剩一遍、内容完整。
            // 其他终端：打印 previousScreen（真实的最后一帧），清屏防 scrollback 残留。
            if (this.imageProtocol === "kitty") {
                // 退出 alt screen 后 clear：\x1b[H 光标归位 + \x1b[2J 清屏 + \x1b[3J 清 scrollback
                this.terminal.write(`${BEGIN_SYNCHRONIZED_OUTPUT}${EXIT_ALT_SCREEN}\x1b[?25h${END_SYNCHRONIZED_OUTPUT}\x1b[H\x1b[2J\x1b[3J`);
            }
            else {
                const width = Math.max(1, this.terminal.columns);
                this.lastDocument = (this.previousScreen ?? []).map((line) => line.replaceAll(CURSOR_MARKER, ""))
    ...
    handleViewportInput(data) {
        ...
        if (data === FOCUS_IN) {
            // ISSUE 079 配套：长时间休眠卸载后，窗口回到前台即自动重建消息区——
            // 只加载不唤醒（agent 保持 hibernated，status bar 照常），用户不发消息也能看到历史。
            try { globalThis.__genshinOnTuiFocusIn?.(); } catch (e) { console.error("[god.frontend.tui/overrides/pi-tui/tui-alt-screen.js] " + (e?.message || e)); }
            return { consume: true };
        }
    ...
    routeWheel(event) {
        ...
        //   - 新序列（间隔≥250ms）回基线 1
        const now = Date.now();
        const dt = this._lastWheelTime ? now - this._lastWheelTime : 0;
    ...
    copySelectionToClipboard() {
        ...
        if (text.length === 0)
            return;
        // ISSUE 132 (2026-09-06 fork patch)：OSC 52 前优先系统剪贴板工具。原实现仅发 OSC 52——
