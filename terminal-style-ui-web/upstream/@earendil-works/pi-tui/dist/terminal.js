...
export class ProcessTerminal {
    ...
    setupStdinBuffer() {
        ...
        this.stdinBuffer.on("data", (sequence) => {
            if (process.env.INPUTPS03 === "0") return; // fork: 输入协议调试开关（0.80.7 时代定制）
        ...
        this.stdinDataHandler = (data) => {
            if (process.env.INPUTPS02 === "0") return; // fork: 输入协议调试开关
    ...
    queryAndEnableKittyProtocol() {
        ...
        if (process.env.INPUTPS01 !== "0") process.stdout.write(KITTY_KEYBOARD_PROTOCOL_QUERY); // fork: 输入协议调试开关
    }
    ...
    forwardInputSequence(sequence) {
        if (!this.inputHandler)
            return;
        if (process.env.INPUTPS04 === "0") return; // fork: 输入协议调试开关
    ...
    write(data) {
        // fork 2026-09-13：全角序号 ①②③ 的"两格契约"输出侧——排版按 2 格算，终端只给 1 格时在序号后补一个真实空格（utils.js 注释）
