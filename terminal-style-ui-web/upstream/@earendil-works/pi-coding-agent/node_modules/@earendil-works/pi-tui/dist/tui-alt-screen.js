...
export class TuiAltScreen extends TuiBase {
    ...
    beforeTerminalStart() {
        ...
        // 2026-09-23 定稿：图片渲染暂不支持（先禁用）。完整调研见 EXPERIENCE 文档。
        // 无条件禁用所有图片协议：iTerm2 inline image 无 ID 在 alt-screen 重绘下不可管理；
        // kitty 协议虽可管理，但 iTerm2 的 kitty 实现有 cell 尺寸换算 artifacts + 图片顶格（encodeKitty 无 x 偏移）不美观。
        ...
        // 2026-09-25（ISSUE 285，反馈："自动复制也坏了"）：终端状态只在 stop() 里复原；任何绕过 stop 的退出
    ...
    handleViewportInput(data) {
        ...
        if (data === FOCUS_IN) {
            ...
            try { globalThis.__genshinOnTuiFocusIn?.(); } catch (e) { console.error("[god.tui/overrides/pi-tui/tui-alt-screen.js] " + (e?.message || e)); }
            return { consume: true };
        }
