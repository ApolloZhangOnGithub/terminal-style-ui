...
import { CURSOR_MARKER, compositeTuiLine, renderErrorLog } from "./tui.js";
...
function renderCached(context, component, width) {
    ...
    if (!lines) {
        // fork: 布局测量渲染同样做异常隔离（D0102），否则一个坏组件
        // 会从 renderCached 一路 uncaughtException 杀掉整个 TUI 进程
        try {
            lines = component.render(safeWidth);
        }
        catch (e) {
            renderErrorLog("D0102", e?.stack || String(e));
            lines = [];
        }
