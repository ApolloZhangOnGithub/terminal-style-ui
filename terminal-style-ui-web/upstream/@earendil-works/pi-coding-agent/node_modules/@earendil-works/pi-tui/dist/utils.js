...
function graphemeWidth(segment) {
    ...
    // fork 2026-09-17（ISSUE 261）：Windows 系终端（win32 / WSL / Windows Terminal）+ CJK 字体
...
/**
 * fork 2026-09-17（ISSUE 261）：ambiguous/neutral 符号宽度判据。
...
export function visibleWidth(str) {
    ...
    // Check cache
    // fork 2026-09-17（ISSUE 261）：缓存键带上 ambiguous 判据（1|2）——探测回包后判据变，
...
function wrapSingleLine(line, width) {
    ...
    for (const token of tokens) {
        ...
        if (tokenVisibleLength > width && !isWhitespace) {
            let longToken = token;
            if (currentLine) {
                // 2026-09-24（反馈：URL 折行多空格）：leading 空格（缩进）单独成 currentLine 时，长 token 会把纯空格 push 成空行。
 ...
 * 2026-09-22（反馈：URL/path/标识符被逐字硬切中间断）。
 */
...
function breakLongWord(word, width, tracker) {
    ...
    // 2026-09-22（反馈：长 token 折行硬切词中间断——URL/path/标识符被逐字硬切）：
